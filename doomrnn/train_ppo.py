...
def main(args):
  ...
  if args.beta:
    global filebase
    filebase = "beta_" + filebase + "_{}".format(args.name)
    # load the correct model
    model.env.vae.load_json(os.path.join("tf_beta_vae", '{}.json'.format(args.name)))
    model.env.rnn.load_json(os.path.join("tf_beta_rnn", '{}.json'.format(args.name)))
    total = 201
  ...
  if (rank == 0):
    master(total)
    comm.Abort()
    # call python -m mpi4py train.py --flags to exit automatically
...
if __name__ == "__main__":
  ...
  parser.add_argument('--beta', action='store_true', help='beta experiments. will load models from tf_beta_vae/rnn')
  parser.add_argument('--name', type=str, default="vae", help='name of the models without ".json"')
